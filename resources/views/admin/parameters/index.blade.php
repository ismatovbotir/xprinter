...
<div class="page-header">
  <div>
    <div class="page-title">Parametrlar</div>
    <div class="page-subtitle">Jami {{ $parameters->total() }} ta parametr</div>
  </div>
...
<form method="GET" action="{{ route('admin.parameters.index') }}" class="filter-bar">
  <div class="filter-search">
    <svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
    <input type="text" name="search" value="{{ request('search') }}" placeholder="Nomi bo'yicha qidirish...">
  </div>
  @if(request('search'))
    <a href="{{ route('admin.parameters.index') }}" class="btn btn-ghost">Tozalash</a>
  @endif
</form>
    ...
    <table>
      <thead>
        <tr>
          <th>Nomi (UZ)</th>
          <th>Nomi (RU)</th>
          <th style="width:130px;text-align:center">Kategoriyalar</th>
      ...
      <tbody>
        ...
        <tr>
          <td style="font-weight:600;color:var(--ink)">
            {{ $parameter->translations->firstWhere('lang','uz')?->name ?? '—' }}
          </td>
          <td style="color:var(--ink-soft)">
            {{ $parameter->translations->firstWhere('lang','ru')?->name ?? '—' }}
          </td>
          <td style="text-align:center">
            <span style="font-family:'JetBrains Mono',monospace;font-size:13px;font-weight:600;color:var(--blue)">
              {{ $parameter->categories_count }}
            </span>
          </td>
          <td>
            <div class="actions-cell">
              <a href="{{ route('admin.parameters.edit', $parameter) }}" class="action-btn" title="Tahrirlash">
              ...
              <form method="POST" action="{{ route('admin.parameters.destroy', $parameter) }}"
                    onsubmit="return confirm('{{ $parameter->translations->firstWhere('lang','uz')?->name ?? 'Parametr' }}ni o\'chirishni tasdiqlaysizmi?')">
                @csrf @method('DELETE')

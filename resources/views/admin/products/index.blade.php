...
<form method="GET" action="{{ route('admin.products.index') }}" class="filter-bar">
  <div class="filter-search">
    ...
    <input type="text" name="search" value="{{ request('search') }}" placeholder="Model raqami yoki nomi bo'yicha...">
  </div>
  ...
  @if(request('search') || request('category_id'))
    <a href="{{ route('admin.products.index') }}" class="btn btn-ghost">Tozalash</a>
  @endif
</form>
        ...
        <tr>
          ...
          <td>
            <div style="font-weight:600;color:var(--ink)">
              {{ $product->translations->firstWhere('lang','uz')?->name ?? '—' }}
            </div>
            <div style="font-size:12px;color:var(--muted)">
              {{ $product->translations->firstWhere('lang','ru')?->name ?? '' }}
            </div>
          </td>
          <td>
            <span style="font-size:12px;background:var(--blue-soft);color:var(--blue-deep);padding:3px 10px;border-radius:6px;font-weight:600">
          ...
          <td>
            <div class="actions-cell">
              <a href="{{ route('admin.products.edit', $product) }}" class="action-btn" title="Tahrirlash">
              ...
              <form method="POST" action="{{ route('admin.products.destroy', $product) }}"
                    onsubmit="return confirm('{{ $product->model_number }}ni o\'chirishni tasdiqlaysizmi?')">
                @csrf @method('DELETE')
